...
—◉ *Contacto para apelaciones:* wa.me/573146224366
╚═════════════════════╝
               `.trim();
              m.reply(messageText);
...
if (action === "add") {
  m.conn.sendMessage(id, {
    sticker: { url: "./src/bienvenida.webp" },
    sticker: { url: "./src/bienvenida2.webp" },
  });
}
if (
  userPrefix &&
  ...
  await m.conn.sendMessage(
    id,
    {
      text: `*[❗] @${
        user.split("@")[0]
      } ᴇɴ ᴇsᴛᴇ ɢʀᴜᴘᴏ ɴᴏ sᴇ ᴘᴇʀᴍɪᴛᴇɴ ɴᴜᴍᴇʀᴏs ᴀʀᴀʙᴇs ᴏ ʀᴀʀᴏs, ᴘᴏʀ ʟᴏ ϙᴜᴇ sᴇ ᴛᴇ sᴀᴄᴀʀᴀ ᴅᴇʟ ɢʀᴜᴘᴏ*`,
      mentions: [user],
    },
    { quoted: fkontak2 },
  );
    ...
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        // await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
         const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;𝕆𝕔𝕙𝕠𝕒;;;\nFN:𝕆𝕔𝕙𝕠𝕒\nORG:𝕆𝕔𝕙𝕠𝕒\nTITLE:\nitem1.TEL;waid=573146224366:+57 314 622 4366\nitem1.X-ABLabel:𝕆𝕔𝕙𝕠𝕒\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:𝕆𝕔𝕙𝕠𝕒\nEND:VCARD`;
        await this.sendMessage(nk.from, {contacts: {displayName: '𝕆𝕔𝕙𝕠𝕒', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  ...
  const aa = {quoted: m, userJid: conn.user.jid};
    const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '[ ⚠ ] 𝐀𝐕𝐈𝐒𝐎 - 𝐀𝐋𝐄𝐑𝐓𝐀', body: '𝕏𝕒𝕧𝕪𝔹𝕠𝕥-𝕄𝔻', thumbnail: imagen1, sourceUrl: 'https://github.com/Ochoa21/XavyBot-MD'}}}}, aa);
