...
${tradutor.texto1[1]} ${messageNumber}/3
 ...
 ${tradutor.texto1[4]}`.trim();
        ...
        const ochoa = `${plugin.botAdmin || plugin.admin || plugin.group || plugin || noPrefix || hl || m.text.slice(0, 1) == hl || plugin.command}`;
        if (adminMode && !isOwner && !isROwner && m.isGroup && !isAdmin && ochoa) return;
